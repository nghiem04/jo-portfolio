...
const caseStudies = [
  {
    ...
    overview: `Opal had been designed for ticketing administration, not commuting. Research with [N] participants surfaced a structural fragmentation pattern: every commuter was running a three-app workflow — TripView for real-time status, Google Maps for routing, Opal only to top up. None of those switches were preferences; each was an adaptation to a gap the app did not fill. A second structural problem compounded this: the app had no awareness of routine. Most participants made the same journey every day, but the app presented the same blank state every session. The redesign consolidated the workflow into a single surface, rebuilt around how Sydney commuters actually begin their day.`,
